...
export class Collection<T> {
    ...
    concat(...items: (T | Collection<T>)[]): Collection<T> {
        const flattenedItems = items.map(item => (item instanceof Collection ? item.toArray() : item));
    ...
    unzip<U>(): Collection<U[]> {
        ...
        const unzipped = (this.xs[0] as U[]).map((_, i) => this.xs.map(tuple => (tuple as U[])[i] as U));
